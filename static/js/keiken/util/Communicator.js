  ...
  function (declare) {
    var Communicator = declare(null, {
      ...
      subscribe: function (events, handler, name, context, counter) {
        ...
        var instance = this
        var eventObjectList = this._parse(events, handler, name, context, counter)
      ...
      unsubscribe: function (events, handler) {
        this._assertEventsInput(events)
        var instance = this
        var eventList = this._parseEventString(events)
      ...
      _removeEvent: function (event) {
        ...
        var type = this._getType(typeAndName[0])
        var name = this._getName(typeAndName[0], typeAndName[1])
        if (type && name) {
      ...
      _triggerName: function (name, data) {
        var instance = this; var hasName = false
        name = name.substring(1)
        $.each(this.events, function (type, nameList) {
          if (nameList.hasOwnProperty(name)) {
      ...
      _triggerEvent: function (event, data) {
        var typeAndName = this._separateTypeAndName(event)
        var namedCollection = this._getName(typeAndName[0], typeAndName[1])
        var instance = this
      ...
      _parse: function (events, handler, name, context, counter) {
        var eventList = []
        var instance = this
        var eventOptions = this._extractEventOptions(handler, name, context, counter)
        var eventTypes
      ...
      _parseEventString: function (events) {
        var eventList; var instance = this
      ...
      _extractEventOptions: function (arg1, arg2, arg3, arg4) {
        ...
        var argsTypes = args.map(function (arg) {
          return typeof arg
        })
        var eventObject = {}
      ...
      _createEventFromString: function (event, eventOptions) {
        this._assertIncludesType(event)
        var typeAndName = this._separateTypeAndName(event)
        var eventObject = $.extend({}, eventOptions)
      ...
      _assertEventAndHandler: function (events, handler) {
        var valid
        if (handler) {
          valid = !!((typeof events === "string" && typeof handler === "function"))
        } else {
          valid = !!((typeof events === "function" || typeof events.handler === "function"))
        }
        if (!valid) {
      ...
      _assertEventsInputOptions: function (options) {
        var valid = true; var wrongAttribute; var wrongValue
        if (typeof options !== "function") {
